                    ...
                    {plantsArray.map((category, index) => {
                        return (
                            <div key={`category-${index}`}>
                                <h1 className="product-list plant-heading">
                                    {category.category}
                                </h1>
                                <div className="product-list">
                                    {category.plants.map((plant, subIndex) => (
                                        <div className="product-card" key={`${plant.name}-${subIndex}`}>
                                            <h3 className="product-title">{plant.name}</h3>
                                            <img className='product-image' src={plant.image} alt={`Image of ${plant.name}`} />
                                            <h3 className="product-price">{plant.cost}</h3>
                                            <i><p>{plant.description}</p></i>
                                            <button className={addedToCart[plant.name] ? "product-button product-button-disabled" : "product-button"} onClick={() => handleAddToCart(plant)} disabled={addedToCart[plant.name]}>{addedToCart[plant.name] ? 'Added To Cart' : 'Add To Chart'}</button>
                                        </div>
                                    ))}
                                </div>
                            </div>
